...
public class WebSocketVerticle extends AbstractVerticle {
  ...
  private void handleWebSocket(ServerWebSocket ws) {
     ...
     */
    /*
     * String token = ws.headers().get("Authorization"); if (!isValidToken(token)) {
     * log.warn("Unauthorized WebSocket connection attempt from {}",
     * ws.remoteAddress()); connectionCount.decrementAndGet(); ws.reject(401);
     * return; }
     */

    String clientId = ws.query();

    try {
      log.info(
          "Client connected ({}/{}): clientId={}, address={}",
          currentConnections,
          MAX_CONNECTIONS,
          clientId,
          ws.remoteAddress());
    ...
    if (!ws.isClosed()) {
      ws.writePing(
          ...
          result -> {
            if (result.failed()) {
              log.warn(
                  "Failed to send ping to client {}: {}", clientId, result.cause().getMessage());
  ...
  private void broadcastToAllClients(Message<byte[]> msg) {
    ...
    try {
      ...
      log.debug(
          "Broadcast complete: inquiryId={}, sent={}, failed={}",
  ...
  private boolean sendToClient(ServerWebSocket ws, Buffer buffer) {
    ...
    try {
      ...
      log.error("Exception sending message to client {}: {}", ws.textHandlerID(), e.getMessage());
      return false;
    }
  }
  ...
  private void cleanupConnection(String clientId, ServerWebSocket ws) {
    ...
    log.debug(
        "Connection cleaned up: clientId={}, remaining={}", clientId, activeConnections.size());
  }
}
